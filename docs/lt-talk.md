...
## ⑤.5 コラム：「外部からできる」の正体（巻けるなら飛ばしてOK）

> ここで聴衆が「え、そんなの外部からできるの!?」とザワつくので、種明かし。刺さるパート。

**実は「外部から」ではない。「ユーザーが正式に内側へ招き入れている」。**

- これができるのは、**ユーザーが拡張をインストールし、`https://www.instagram.com/*` への権限を許可した**から。
  その瞬間、ブラウザが拡張のコードを**そのページの内側で特権実行**してくれる。**外から殴ってるのではなく、通行証をもらって内側に入って改造している**。

**だから“普通のサイト”には絶対できない。**

- ランダムな Web サイトが、別タブの Instagram の `JSON.parse` を書き換える——は **same-origin policy** で**不可能**。
- できるのは **インストール済みで、対象オリジンの権限を持つ拡張機能だけ**。ここが Web の信頼境界線。

**＝ 拡張機能は強力すぎる、という話でもある。**

- 同じ仕組みで、悪意ある拡張なら **入力フォーム・Cookie・表示内容の盗み見/改変** もできてしまう。
- **だから**「インストール時に権限が表示される」「Firefoxは署名を要求する」。前半の署名の話と全部つながる。
- 殺し文句: **「拡張機能 = ユーザーが自分で招き入れた、ページ内で動く MITM（中間者）」**。

**おまけ：ページ側も抵抗できる（軍拡競争）。**

- Instagram が本気なら `Object.freeze(JSON)` で上書き防止、改変検知、JSON難読化…で**対策可能**。
- なので方式Bは「**今は効くが相手が動けば壊れる**」前提 → **方式A（DOM掃除）をフォールバックに残す**設計判断につながる。
...
- そして: **これができるのは「拡張機能 = ユーザーが招き入れたページ内MITM」だから。便利さの裏返しで強力すぎる → だから権限表示と署名がある**。
