...
自分でビルドしなくても、最新の `.xpi` をそこからダウンロードできます。

> ⚠️ 公開される `.xpi` は **未署名** です。通常版 Firefox では恒久インストールがブロックされるため、
> `about:debugging` からの「一時的なアドオン」として読み込んでください（8.2 と同じ手順で、
> `manifest.json` の代わりにダウンロードした `.xpi` を選択）。恒久インストールには AMO 署名が必要です。
